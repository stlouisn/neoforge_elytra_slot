...
import dev.elytraslot.common.CurioElytra;
...
public class ElytraSlot {
  ...
  public ElytraSlot(IEventBus eventBus) {
    ...
    eventBus.addListener(FMLClientSetupEvent.class, (evt) -> this.clientSetup(eventBus));
  ...
  private void clientSetup(final IEventBus eventBus) {
  ...
  private void playerTick(final PlayerTickEvent.Post evt) {
    ...
    if (attributeInstance != null) {
      ...
      if (!attributeInstance.hasModifier(CurioElytra.ELYTRA_CURIO_MODIFIER.id()) && ElytraSlotCommon.canFly(player)) {
